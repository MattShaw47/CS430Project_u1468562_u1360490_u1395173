...
class CloudDrawingRepository(
    ...
    suspend fun shareDrawingWithUser(localId: Long, drawing: DrawingImage, userEmail: String) {
        ...
        try {
            ...
            db.collection("users").document(recipientUid).collection("received").document("$localId.png").set(recipientDrawingData).await()
            db.collection("users").document(currUid).collection("shared").document("$localId.png").set(senderDrawingData).await()
